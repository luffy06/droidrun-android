...
<Note>
OpenAI and OpenAI-compatible endpoints are included by default. For Google Gemini or additional providers, install extras such as `droidrun[google]`, `droidrun[anthropic]`, or `droidrun[deepseek]`.
</Note>
...
```bash
# Using default configuration (OpenAI)
...
# Configure api_base and api_key in config.yaml under the selected llm_profiles entry,
# then run with provider OpenAILike.
droidrun run "Summarize the current screen" --provider OpenAILike --model MiniMax-M1 --api-base https://api.minimax.chat/v1
